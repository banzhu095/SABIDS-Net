...
def parser() -> argparse.ArgumentParser:
    ...
    select = sub.add_parser("select-roi"); select.add_argument("--input-root", type=Path, required=True); select.add_argument("--selection-list", type=Path, required=True); select.add_argument("--output-root", type=Path, required=True); select.add_argument("--roi-size", type=int, choices=(32,48,64), default=48); select.add_argument("--unlock-rois", action="store_true"); select.add_argument("--unlock-reason", default=""); select.add_argument("--reset-rois", action="store_true", help="Archive the current registry and begin a blank blinded selection; requires explicit unlock reason.")
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.command == "discover":
        ...
        select_rois(args.input_root, args.selection_list, args.output_root, args.roi_size, args.unlock_rois, args.unlock_reason, args.reset_rois)
