...
def collect_predictions(
    ...
    for batch in tqdm(loader, desc="Collect validation probabilities"):
        ...
        probability = output["vessel_prob"].cpu().numpy()
...
def main() -> None:
    ...
    dataset = OCTManifestDataset(
        ...
        datasets=config["data"].get("val_datasets"),
    )
    workers = (
        args.num_workers
    ...
    pd.DataFrame(rows).to_csv(
        output_dir / "vessel_threshold_sweep.csv",
        index=False,
        encoding="utf-8-sig",
    )
    result = {
    ...
    write_json(result, output_dir / "best_vessel_threshold.json")
