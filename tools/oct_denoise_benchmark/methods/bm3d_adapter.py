...
def bm3d_adapter(image: np.ndarray, config: Mapping[str, Any], context: AdapterContext) -> np.ndarray:
    ...
    if method_id == "bm3d_standard" and stage_name in {"hard", "hard_thresholding"}:
        raise ValueError("bm3d_standard requires hard-thresholding and Wiener stages")

    # Validate protocol invariants before importing the optional runtime.  This
    # keeps configuration errors deterministic even in smoke-test environments
    # where the BM3D package is intentionally absent.
    import bm3d
