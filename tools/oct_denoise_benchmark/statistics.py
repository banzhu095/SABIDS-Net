...
def paired_differences(position: pd.DataFrame) -> pd.DataFrame:
    ...
    if "split" not in position: position["split"] = "unspecified"
    ...
    requested += [("dncnn_paired", "sabids_current"), ("nafnet_paired", "sabids_current"), ("tcfl_dncnn", "sabids_current"),
                  ("dncnn_paired", "tcfl_dncnn"), ("nafnet_paired", "tcfl_dncnn")]
    for (dataset, split), dataset_rows in position.groupby(["dataset", "split"]):
        ...
        for baseline, method in dict.fromkeys(requested):
            ...
            for metric in metrics:
                ...
                rows.append({"dataset": dataset, "split": split, "baseline_method": baseline, "method_id": method, "metric": metric, "n_positions": len(common), "mean_paired_difference": float(values.mean())})
...
def bootstrap_confidence_intervals(position: pd.DataFrame, iterations: int = 10_000, seed: int = 42) -> pd.DataFrame:
    ...
    metrics = [column for column in METRIC_COLUMNS if column in position]
    rows = []
    for (dataset, split, method), group in position.groupby(["dataset", "split", "method_id"]):
        ...
        for metric in metrics:
            ...
            rows.append({"comparison_type": "method_mean", "dataset": dataset, "split": split, "baseline_method": "", "method_id": method, "metric": metric, "n_positions": len(values), "mean": float(values.mean()), "ci95_low": float(low), "ci95_high": float(high), "iterations": iterations, "seed": seed})
        ...
        for baseline, method in dict.fromkeys(requested):
            ...
            for metric in metrics:
                ...
                if len(values) == 1:
                    ...
                    samples = values[indices].mean(axis=1)
                    low, high = np.quantile(samples, [0.025, 0.975])
                rows.append({"comparison_type": "paired_difference", "dataset": dataset, "split": split, "baseline_method": baseline, "method_id": method, "metric": metric, "n_positions": len(values), "mean": float(values.mean()), "ci95_low": float(low), "ci95_high": float(high), "iterations": iterations, "seed": seed})
    return pd.DataFrame(rows)
