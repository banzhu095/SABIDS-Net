...
## 0.1 Stage 2 E0--E3诊断实现

针对稳定修复后仍可能出现的“早期达到最佳、随后退化”，代码已增加三条
严格隔离的Stage 2路径：E1完全冻结Stage 1上游及去噪功能并关闭UGBI；
E2只开放来源detach的去噪→分割接收侧；E3在E2基础上使用真值层ROI内
masked BCE+Dice，并保留层外containment。训练会额外保存epoch 0、无增强
train-eval、逐组指标、整层/空掩膜基线、参数审计和清单/初始化指纹。

本地真实清单CPU smoke仅证明E1训练链路可运行，且固定去噪探针漂移为0；
