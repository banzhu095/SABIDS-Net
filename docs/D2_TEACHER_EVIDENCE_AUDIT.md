...
| train positions | 13 positions | 30 positions | **different** | config/runtime vs active lock |
...
checkpoint cannot receive a derived binding because its train cohort differs,
the locked hashes were never recorded, and its historical label inventory
inspected sealed-test label paths. Adding a current `protocol_id` would not
repair those missing training-time facts.
