...
# 2026-09-01 — Cloud presentation archive implementation (no performance run)

- Added `tools/build_presentation_archive.py`, a four-phase audit/evaluate/assemble/package workflow for complete server assets.  Missing validation inference is gated by `--run-missing-validation`; it calls only existing validation/export entry points and never a training mode.
- Added fixed same-window Stage 1 triptychs, current-metric recomputation from float predictions, requested green-layer/red-vessel overlays, TP/FP/FN maps, P2a/P2b separation, fixed Stage 2/Joint/input atlas collection, literature `SOURCE REQUIRED` handling, 15 PNG/PDF presentation figures, manifest validation, and full/GPT packaging.
- Added `tools/build_presentation_workbooks.mjs` for six artifact-tool workbooks, including `SABIDS_presentation_results.xlsx`.  Packaging refuses a missing master workbook unless `--skip-workbooks` is explicit.
- Local audit smoke inventoried only the Windows repository's available smoke artifacts (33 candidate run paths, 18 checkpoint files, no formal prediction directories) with `test_assets_opened=0`.  This is not a cloud performance result.  Three focused overlay/test-boundary tests passed; full cloud inference remains to run against `/mnt/SABIDS-Net`.
